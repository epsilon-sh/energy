...
const EnergyDashboard: React.FC = () => {
  ...
  const spotIncurred = consumption?.data?.reduce((acc: CostRecord[], { quantity, resolution }, idx) => {
  ...
  const fixedIncurred = consumption.data?.reduce((acc: CostRecord[], { quantity, resolution }) => {
    const periodCost = calculatePrice(
      fixedContract,
      +quantity,
      resolution as Duration,
  ...
  const handleEndChange = (date: Date): void => {
    ...
    setSearchParams(prev => {
      prev.set("end", date.toISOString());
      return prev;
    });
  };

  const handleResolutionChange = (resolution: Duration): void => {
    setSearchParams(prev => {
  ...
  const getErrorMessage = (error: unknown, fallback = ""): string | undefined => {
  ...
  const dateToLocalInputString = (date: Date): string => {
