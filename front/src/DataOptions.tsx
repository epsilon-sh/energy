...
const DataInfo = ({ data }: { data: ConsumptionData[] }) => {
    ...
    const info = {
        Meter: data[0].meteringPoint, // FIXME: undeclared property
        Start: data[0].startTime.toDateString(), // FIXME: undeclared property
        End: data[data.length - 1].startTime.toDateString(), // FIXME: undeclared property
        Count: data.length,
        Total: data.reduce((acc: number, m: typeof data[0]) => +m.quantity + acc, 0).toFixed(),  // FIXME: undeclared property
    };
...
const FilesInput = ({ handleChange, uploading }: { handleChange: (_files: FileList | null) => void, uploading: boolean }) => {
